<?php
	...
	$query = $conn->prepare('SELECT d.id as departmentID, d.name AS departmentName, l.id as locationID, l.name AS locationName FROM department d LEFT JOIN location l ON (d.locationID = l.id) WHERE d.name LIKE ? ORDER BY d.name');
